...
def inference(images):
  ...
  # pool1
            # focus on the center-most dim. for ksize & strides
  pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                         padding='SAME', name='pool1')
  print ("pool1",pool1.get_shape())
  # conv2
  with tf.variable_scope('conv2') as scope:
                                                      # make sure to update channel size
...
#############################END OF ENCODING PART##################################
  ...
  up4=upsample(pool4,pool=pool4,original=conv4,scale=2,pad_out_h=True)

  #upsampling 4
  #with tf.variable_scope('up4') as scope:
    #kernel = _variable_on_cpu('up4', [2,2,64,64], tf.truncated_normal_initializer())
  #  kernel = _variable_with_weight_decay('up4', shape=[2, 2, 64, 64],
  #                                       stddev=1/float(2*2*64), wd=0.0)
  ...
  with tf.variable_scope('de_conv4') as scope:
    kernel = _variable_with_weight_decay('weights', shape=[7, 7, 64, 64],
                                         stddev=1/float(7*7*64), wd=WD)
    de_conv = tf.nn.conv2d(up4, kernel, strides=[1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.0))
  ...
  #  kernel = _variable_with_weight_decay('up3', shape=[2, 2, 64, 64],
  #                                       stddev=1/float(2*2*64), wd=0.0)
  ...
  up3=upsample(de_conv4,pool=pool3,original=conv3,scale=2)
  ...
  #  kernel = _variable_with_weight_decay('up2', shape=[2, 2, 64, 64],
  #                                       stddev=1/float(2*2*64), wd=0.0)
  ...
  up2=upsample(de_conv3,pool=pool2,original=conv2,scale=2)
  ...
  #with tf.variable_scope('up1') as scope:
   ...
   # kernel = _variable_with_weight_decay('up1', shape=[2, 2, 64, 64],
   #                                      stddev=1/float(2*2*64), wd=0.0)
  ...
  up1=upsample(de_conv2,pool=pool1,original=conv1,scale=2)
